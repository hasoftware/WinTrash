...
### Thêm
- **Xác minh bản quyền CHÍNH HÃNG** (trong `-Action license`): gọi API chính chủ Microsoft `SLIsGenuineLocal` (Slwga.dll — kiểm tra cờ Tampered của giấy phép, đúng cơ chế Windows tự xác minh) kết hợp `LicenseStatus` WMI (7 trạng thái: Licensed/OOB/OOT/Non-Genuine/Notification...) và key OEM nhúng BIOS (OA3). Hiện bảng tình trạng: phiên bản Windows, trạng thái kích hoạt, xác minh Microsoft, kênh cấp phép (kèm partial key), key OEM trong BIOS. Kết luận tổng 3 mức: **CHÍNH HÃNG** (xanh) / **chính hãng nhưng còn dấu vết tool lậu** (vàng — khuyên gỡ sạch) / **CHƯA có bản quyền hợp lệ** (đỏ). Khi chưa chính hãng, in khuyến nghị mua bản quyền: chỉ dẫn nguồn chính thức của Microsoft (microsoft.com / bản cài sẵn theo máy của nhà sản xuất — không quảng cáo bên thứ ba), cảnh báo key giá rẻ trôi nổi có thể bị thu hồi, cách tự kiểm tra qua Settings > Activation, và lưu ý pháp lý Việt Nam (NĐ 131/2013/NĐ-CP + NĐ 341/2025/NĐ-CP hiệu lực 15/02/2026). SLWGA offline hoặc API không gọi được thì rơi êm về trạng thái kích hoạt; không đọc được cả hai nguồn thì kết luận thận trọng "chưa có bản quyền hợp lệ".
- **Kiểm tra bản quyền Windows** (menu mới + `-Action license`): kiểm toán CHỈ ĐỌC 8 hạng mục, không thay đổi gì trên máy — (1) **KMS Crack**: phát hiện máy chủ KMS lậu cấu hình trong `SoftwareProtectionPlatform` (loopback = trình giả lập chạy tại chỗ, danh sách máy chủ KMS công cộng lậu, KMS thủ công trên máy ngoài domain); (2) **MAS / HWID**: quét lịch sử PSReadLine của mọi user + hộp thoại Run tìm lệnh kích hoạt lậu (`massgrave`, `get.activated.win`, `kms38`, `hwidgen`, `slmgr /ipk`...) — trả về "lịch sử dòng lệnh sạch / không sạch"; (3) **KMS38 Hook**: đối chiếu cấu trúc thời hạn — hạn KMS vượt 180 ngày chuẩn (đẩy tới 2038) hoặc "vĩnh viễn trên kênh KMS" (TSforge/ZeroCID) đều là không hợp lệ; (4) **Logic bản quyền**: đối chiếu kênh cấp phép (Retail/OEM/Volume) với key OEM nhúng BIOS (OA3) và tư cách domain — BIOS có key OEM mà kích hoạt Volume, hoặc kênh Volume trên máy cá nhân = không hợp lệ; (5) **Thư mục tool lậu**: KMSpico/KMSAuto/KMS_VL_ALL/Microsoft Toolkit... trong Program Files/ProgramData + tàn dư `SECOH-QAD.exe`, `SppExtComObjHook.dll` trong thư mục Windows; (6) **Tác vụ ẩn**: task gia hạn lậu trong Task Scheduler (khớp cả tên lẫn lệnh thực thi); (7) **Can thiệp Registry**: khóa `NoGenTicket = 1` chặn hệ thống gửi ticket kiểm tra bản quyền, dịch vụ `sppsvc`/`ClipSVC` bị vô hiệu, IFEO đánh tráo `sppsvc.exe`/`slui.exe`; (8) **Máy chủ không an toàn**: tệp hosts chặn/chuyển hướng máy chủ kích hoạt Microsoft + kết nối TCP đang mở tới cổng KMS 1688. Kết quả từng hạng mục hiện PHÁT HIỆN / KHÔNG PHÁT HIỆN (hoặc HỢP LỆ / KHÔNG HỢP LỆ) kèm chi tiết, chốt bằng kết luận tổng. Truy vấn WMI cấp phép chỉ chạy một lần dùng chung; menu đủ 4 ngôn ngữ; kèm 15 test Pester cho phần logic thuần (KMS38, đối chiếu kênh/BIOS).

## [1.4.0] - 2026-07-09

### Thêm
- **Dọn RAM 2 chế độ** (menu mới + `-Action ram`): chế độ **NHẸ NHÀNG** chỉ dọn rác RAM — nén working set của các tiến trình và (khi chạy admin) xả standby cache + trang modified qua đường kernel như RAMMap, KHÔNG đóng bất kỳ ứng dụng nào; chế độ **BẤT CHẤP** thêm bước đóng ứng dụng — bạn tick chọn app trong danh sách checkbox (kèm tên cửa sổ + RAM đang chiếm), xác nhận y/N, app được đóng LỊCH SỰ trước (kịp hỏi lưu tài liệu), app nào lì mới hỏi RIÊNG thêm một lần nữa rồi mới buộc dừng. Hiện RAM trống trước/sau và số MB chênh lệch. Không tương tác (`-Action ram` qua lịch/console redirect) luôn chạy chế độ nhẹ. Shell/hạ tầng Windows (explorer, dwm, host console đang chạy script...) không bao giờ nằm trong danh sách đóng.
